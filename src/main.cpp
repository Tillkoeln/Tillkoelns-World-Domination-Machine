...
unsigned int nWorkTargetSpacing = 2 * 60; // 2 Minutes 
...
const string strMessageMagic = "TillkWDM Signed Message:\n";
...
int64_t GetProofOfWorkReward(int64_t nFees)
{
   ...
   if (pindexBest->nHeight <= 1)
	...
	else if (pindexBest->nHeight <= 13371337)  //paid 0 Reward but fee`s 
      {
        int64_t nSubsidy = 0 * COIN;
        return nSubsidy + nFees;
      }      
...
{  
    ...
    if(pindexBest->nHeight > 1)
    {
        nSubsidy = 0.13371337 * COIN; 
		return nSubsidy + nFees; 
    }

	else if(pindexBest->nHeight > 13371337)    // paid 1% per annum 
	{
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "TillkWDM", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
